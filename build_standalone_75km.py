...
<body class="bg-slate-900 text-slate-200 font-sans antialiased h-[100dvh] w-full overflow-hidden flex flex-col md:flex-row">
    ...
    <div id="content-container" class="flex-1 flex flex-col min-w-0 bg-slate-900 z-10 shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.5)] h-[60vh] md:h-screen overflow-hidden">
        ...
        <div class="p-3 md:p-6 pb-0 flex-shrink-0 bg-slate-900 z-20">
            <div class="flex justify-between items-center w-full">
                <div>
                    <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-lime-400 to-cyan-400 mb-0.5">Wyrypa 75km Ultra-Trekking <span style="font-size: 0.5em; color: white;">v1.3</span></h1>
            ...
            <div class="bg-slate-800/50 p-3 md:p-5 rounded-lg border border-slate-700 shadow-xl mb-3 md:mb-4">
                ...
                <div class="flex flex-row flex-wrap gap-2 md:gap-3 items-center mb-4 md:mb-6">
                    ...
                    <div class="flex bg-slate-800 rounded border border-slate-600 overflow-hidden shadow-sm h-[28px] md:h-[38px] items-center">
                        ...
                        <button id="btn-delete-map" class="hidden px-2 md:px-3 text-xs md:text-sm font-bold bg-red-950/40 text-red-400 hover:bg-red-900/60 hover:text-red-200 transition-colors h-full border-l border-slate-600 cursor-pointer" title="Usuń pobraną mapę / Delete downloaded map">
                            🗑️
                        </button>
                        <div id="download-progress-bar" class="hidden h-full bg-slate-900 border-l border-slate-600 px-3 flex items-center text-[10px] md:text-xs font-mono text-cyan-400">
                            0%
                        </div>
                    </div>
    ...
    <script>
        ...
        function handleGpsPosition(pos) {{
            ...
            let dotColor = '#06b6d4'; // cyan accent color for 75km app
            if (accuracy > 100) {{
                dotColor = '#ef4444';
            }} else if (accuracy > 50) {{
                dotColor = '#f97316';
        ...
        function handleGpsWatchSuccess(pos) {{
            const now = Date.now();
            const intervalVal = parseInt(document.getElementById('gps-poll-interval').value);
        ...
        function snapGpsToCheckpoint(cpKm) {{
            ...
            const gpsLatLng = gpxTrackPoints[closestIdx].latlng;
            let lat = gpsLatLng.lat;
            let lon = gpsLatLng.lng;
            let accuracy = gpsCurrentAccuracy || 10;
        ...
        function updateMapButtonsState() {{
            ...
            try {{
                ...
                if (downloaded === '1') {{
                    ...
                    btnDownload.classList.remove('bg-slate-800', 'text-slate-500', 'cursor-not-allowed');
                    btnDownload.classList.add('bg-slate-700', 'text-slate-300', 'hover:text-white');
            ...
            if (btnDelete) {{
                btnDelete.addEventListener('click', async () => {{
                    ...
                    try {{
                        ...
                        showToast(isPl ? "Błąd usuwania mapy" : "Failed to delete offline map");
                    }}
                }});
            }}
